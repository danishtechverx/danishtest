...
function thumbGen($img="",$width=0,$height=0,$arguments=""){
	...
	if(!is_readable($sitePath.$fileCache) or $args['force']){
		...
		if(!isset($bgColor)){ $bgColor=thumbGen_hexToRGB($args['background']); }
		...
		if($imageExtension=="png"){ imagepng($newImage,$sitePath.$fileCache,9); }
		else if($imageExtension=="gif"){ imagegif($newImage,$sitePath.$fileCache); }
		else{ imagejpeg($newImage,$sitePath.$fileCache,70); }
		if($args['effect']=="grayscale" or $args['effect']=="sephia"){
			...
			else{ imagejpeg($newImage,$sitePath.$fileCacheGS,70); }
		}
