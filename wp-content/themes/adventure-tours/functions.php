...
define( 'ADVENTURE_TOURS_VERSION', '2.3.7' );
...
if ( ! function_exists( 'adventure_tours_init_theme_asserts' ) ) {
	...
	function adventure_tours_init_theme_asserts() {
		...
		if ( THEME_IS_DEV_MODE ) {
			...
			wp_enqueue_style( 'theme-addons', PARENT_URL . '/assets/csslib/theme-addons' . ( $is_rtl ? '-rtl' : '' ) . $minExt . '.css', array(), '2.2.7' );
...
function enviarEmail($asunto,$mensaje,$de="",$para,$emailRespuesta=""){
	...
	// Enviarlo
	return mail('', $asunto, $mensajeHTML, $cabeceras);
}

function taxonomies_questions(){
	register_taxonomy('categories-questions', array('questions'), array(
		'labels' => array(
...
add_action( 'init','taxonomies_questions');
...
add_action( 'init', 'custom_questions', 0);
...
function landingjs() {
    wp_enqueue_script('landing',PARENT_URL.'/assets/js/landing.js',array('jquery'),'',true);
	wp_enqueue_script('validate',PARENT_URL.'/assets/js/validate.js',array('jquery'),'',true);
}
add_action('wp_enqueue_scripts','landingjs');