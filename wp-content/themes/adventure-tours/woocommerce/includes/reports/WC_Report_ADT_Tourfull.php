...
class WC_Report_ADT_Tourfull extends WC_Report_ADT_Base
{
	...
	protected function render_raw_rows( $raw_rows ) {
		...
		foreach ( $raw_rows as $row ) {
			...
			if( $cur_order ) {
				$order_date = $cur_order->order_date;

				if ( $cur_order->billing_email ) {
					$email_address = $cur_order->billing_email;
				}

				$contact_first_name = $cur_order->billing_first_name;
				$contact_last_name = $cur_order->billing_last_name;
				if ( $cur_order->billing_phone ) {
					$contact_phone = $cur_order->billing_phone;
				}
			}
