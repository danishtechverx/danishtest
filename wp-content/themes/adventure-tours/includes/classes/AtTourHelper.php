...
class AtTourHelper
{
	...
	public static function get_tour_base_rewrite_rule( $with_front = false, $reset_cache = false ) {
		...
		if ( null === $base_url || $reset_cache ) {
			...
			if ( $tours_page_id && adventure_tours_check( 'is_wpml_in_use' ) && !is_admin() ) {
				if ( null === $cur_lang ) {
	...
	public static function filter_rewrite_rules_array( $rules ) {
		$tour_base_url = self::get_tour_base_rewrite_rule( true, true );
		if ( $tour_base_url && adventure_tours_check( 'woocommerce_active', true ) ) {
